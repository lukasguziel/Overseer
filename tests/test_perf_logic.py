...
def test_dominant_object_is_named_as_the_bottleneck():
    ...
    # 10 of 100 ms is still a tenth of the scene's rebuild — not "light".
    assert out["entries"][1]["level"] == "mid"
...
def test_timer_noise_never_counts_as_a_cost():
    ...
    # Each holds 50% of the total, but 0.1 ms is jitter, not a bottleneck.
