...
## `tokenize(name)`
Splits a name into lowercase word tokens. Splits on whitespace/underscore/hyphen/dot and camel boundaries. Pure-number tokens are dropped (only tokens containing at least one alpha char survive).
