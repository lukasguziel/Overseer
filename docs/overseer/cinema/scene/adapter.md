# cinema SceneAdapter (scene/adapter.py + the per-area mixin modules)
...
One file per area, mirroring `core/`; `scene/adapter.py` composes the mixins,
so `self` spans all of them (state lives in `SceneAdapter.__init__`: `doc`,
`_by_guid`, `_by_sid`, selection sets, `last_changes`):
