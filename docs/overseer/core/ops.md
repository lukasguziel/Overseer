...
- `plan_renames(tree, convention, scope=None, keep=None, dedupe=True)`: renames for non-conforming objects, collision-safe per parent. `scope` limits which guids to rename (None = all); siblings not being renamed reserve their names. Disambiguates against used names.
