// Fake scene for the Playwright e2e tests: a plausible 1.2 GB interior scene
// with 40M polygons, unused materials and heavy textures (copied from the
// readme skill's screenshot fixtures — kept self-contained here so the tests
// never depend on a skill's file layout). Every /api/* request the UI makes
// is answered from this module via e2e/mock.ts — no Cinema 4D involved.
// Keep numbers internally consistent when editing.
