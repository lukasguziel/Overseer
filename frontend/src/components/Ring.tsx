...
// percentage in the UI language, just a 0..100 grade. pct = null means "no
// score" (nothing to judge): empty grey ring, em dash instead of a number.
export default function Ring({ pct, tone, text = true }: { pct: number | null; tone?: Tone; text?: boolean }) {
  const R = 26; const SW = 7; const C = 2 * Math.PI * R
  const v = pct == null ? 0 : Math.max(0, Math.min(100, pct))
  const col = pct == null ? 'var(--panel2)'
    : tone === 'good' ? 'var(--apply)' : tone === 'mid' ? 'var(--warn)' : 'var(--err)'
  return (
    <svg viewBox="0 0 64 64" className="ring">
      ...
      {text && <text x="32" y="37" className="ring-text">{pct == null ? '—' : v}</text>}
    </svg>
  )
}
