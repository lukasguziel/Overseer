...
class TagsAudit(Audit):
    """Object-attachment audit. The base owns the op dispatch, the angle
    helpers and the result shapes (always-on tab: has_any default True); a
    host implements only the read/apply primitives and calls the shared
    helpers via ``self``."""
    ...
    def merge_selection_types(type_list: list, kind_by_type: dict) -> list:
        ...
        for entry in selection:
            ...
            for obj in entry["objects"]:
                ...
                if merged is None:
                    merged = {"guid": obj["guid"], "name": obj["name"],
                              "tags": []}
        ...
        merged_entry = {
            ...
            "label": TagsAudit.SELECTION_LABEL,
    ...
    @staticmethod
    def scan_result(types: dict, missing_phong: list,
                    duplicate_material_tags: list, phong_angles: dict,
                    selection_kinds: dict | None = None,
                    phong: bool = True) -> dict:
        type_list = sorted(types.values(),
                           key=lambda e: (-e["count"], e["label"]))
        if selection_kinds:
            type_list = TagsAudit.merge_selection_types(type_list,
                                                        selection_kinds)
        dominant = TagsAudit.dominant_angle(phong_angles)
