...
from ..core.hostapi.audits import GeneratorsAudit
...
class CinemaGeneratorsAudit(GeneratorsAudit):
    """C4D generator-object settings audit."""

    def _resolve_id(self, attrs):
        for name in attrs:
    ...
    def _read_param(self, obj, param):
        if param["id"] is None:
            try:
                ...
                return None
        try:
            return self._jsonify(obj[param["id"]])
        except Exception:
            return None
    ...
    def _choice_labels(self, obj, pid) -> dict:
        try:
    ...
    def _objects_by_type(self, adapter, tree, progress=None):
    ...
    def scan(self, doc, adapter, tree, progress):
        resolved, groups = self._objects_by_type(adapter, tree, progress)
    ...
    def _members_of(self, adapter, tree, type_id, guids=None):
    ...
    def apply(self, doc, adapter, tree, payload):
        ...
        entry, param = self._entry_and_param(type_key, param_key)
        if entry is None or param is None:
            return {"error": "unknown generator param: %s.%s" % (type_key, param_key)}

        members = self._members_of(adapter, tree, entry["type_id"], guids)
        coerced = self._coerce(value, param["kind"])
    ...
    def select(self, doc, adapter, tree, payload):
        ...
        entry, param = self._entry_and_param(type_key, param_key) if param_key \
            else (self._entry_and_param(type_key, None)[0], None)
        if entry is None:
